...
echo ****START: DataQualityDB Restore****
echo *******************************START: DataQualityDB Restore******************************************* >> %logfile%

echo ****Batch script**** >> %logfile%
echo "sqlcmd -b -S %DataQualityDBservername% -i %~dp0\99.DeploymentScripts\1.RestoreDataQualityBak.sql -v deploymentPath="%~dp0\9 Database BAK File" dbFilePath=%dbFilePath% dbLogPath=%dbLogPath%" >> %logfile%

echo ****SQL Operation**** >> %logfile%
sqlcmd -b -S %DataQualityDBservername% -i %~dp0\99.DeploymentScripts\1.RestoreDataQualityBak.sql -v deploymentPath="%~dp0\9 Database BAK File" dbFilePath=%dbFilePath% dbLogPath=%dbLogPath% >> %logfile%
IF %errorlevel% NEQ 0 @set errorLocation="DataQualityDB Restore"

IF %errorlevel% NEQ 0 GOTO :OnError

echo ****END: DataQualityDB Restore****
echo *******************************END: DataQualityDB Restore******************************************* >> %logfile%
echo . >> %logfile%
