...
IF EXISTS (SELECT 1 FROM [INFORMATION_SCHEMA].[ROUTINES] 
				WHERE SPECIFIC_NAME = 'sGetStandAloneDomainEntity'
				AND SPECIFIC_SCHEMA = 'DQ') 
BEGIN 
	DROP PROC [DQ].[sGetStandAloneDomainEntity]
END

GO

CREATE PROC [DQ].[sGetStandAloneDomainEntity] 
...
**     Desc: Get the entity related to the DQ rule specified
**
**     Return values:
**
**     Called by:
...
BEGIN TRY 
	...
	/* Start Audit*/
	SET @LoadProcess = 'Get Domain Objects @RuleEntityAssociationCode: ' +@RuleEntityAssociationCode
...
/**** START: Apply Value Correction Rules****/
	DECLARE CSR_LoadEntities CURSOR FORWARD_ONLY FOR
		...
		SELECT 	DAE.Name AS DQEntityName,  [Database], [Schema], SourceDatabase, SourceSchema, SourceEntity, EntityName AS CleansingEntity
