...
IF EXISTS (SELECT 1 FROM [INFORMATION_SCHEMA].[ROUTINES] 
				WHERE SPECIFIC_NAME = 'sApplyDQRuleTransformation'
				AND SPECIFIC_SCHEMA = 'DQ') 
BEGIN 
	DROP PROC [DQ].[sApplyDQRuleTransformation]
END

GO

CREATE PROC [DQ].[sApplyDQRuleTransformation] @RuleEntityAssociationCode int
, @ParentLoadId INT = 0 
...
BEGIN TRY 
	...
	FETCH NEXT FROM CSR_RuleTransformation INTO @EntityName, @Databasename, @SchemaName, @EvaluationColumn, @ConvertValuesName, @Length, @Scale, @Precision , @RuleId
		...
		IF @ConvertValuesName in ('Int-To-Varchar', 'Varchar-To-Varchar' )
			...
			/* START: Update those records that Match the value specified in the Source value */
				SET @SQLStmt = 'UPDATE ' +@OuterDatabaseName+ '.'+ @SchemaName + '.' +@EntityName + 
								...
								BEGIN
									SET @SQLStmt = @SQLStmt + ' , '+@OuterStatusColumn+' = ''Cleansed'' '
								END 
		...
		--ELSE IF @ConvertValuesName in ('DateTime-To-Int', 'Varchar(UK)-To-IntDateTime', 'Varchar(US)-To-IntDateTime' )
		...
		--IF @SourceValue = 'NULL'
		--	BEGIN
		--		SET @SQLStmt = @SQLStmt + ' WHERE ' + @EvaluationColumn + ' IS NULL '+ @OuterOptionalFilterClauseWithAND  
		--	END
		--ELSE 
		--	BEGIN
		--		SET @SQLStmt = @SQLStmt + ' WHERE ' + @EvaluationColumn + ' = ''' + @PreferredValue + '''
		...
		EXEC [DQ].[sInsertRuleExecutionHistory] 	
			...
			@RuleSQL = @SQLStmt, 
			@ParentLoadId  = @LoadId,
			@RuleSQLDescription = 'Rules: Transformation - Update the transformed value.'	
		exec (@SQLStmt)
